...
-->
  ...
  <div class="personacharacteristic">
    <characteristic-reference-modal ref="crDialog" :characteristicReference="this.selectedCharacteristicReference" :characteristic="this.objt.theCharacteristic" v-on:characteristic-reference-modal-update="updateCharacteristicReference"/> 
...
export default {
  ...
  methods: {
    ...
    onCommit(evt) {
      evt.preventDefault();
      if (this.checkForm()) {
        this.$emit('persona-characteristic-commit',this.objt);
      }
    },
    onCancel(evt) {
      evt.preventDefault();
